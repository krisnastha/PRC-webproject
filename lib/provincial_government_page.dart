...
class ProvincialGovernmentPage extends StatefulWidget {
  const ProvincialGovernmentPage({super.key});
  @override
  State<ProvincialGovernmentPage> createState() => _ProvincialGovernmentPageState();
}

class _ProvincialGovernmentPageState extends State<ProvincialGovernmentPage> {
  // Language toggle: 0 = English, 1 = Nepali
  final List<bool> _selectedLanguage = [true, false];
  // Google Map initial position (Kathmandu)
  int _selectedIndex = -1; // Track which item is selected
  ...
  Widget build(BuildContext context) {
                ...
                Container(
                  ...
                  child: Row(
                    ...
                    children: [
                      InkWell(
                        onTap: () async {
    final Uri facebookUrl = Uri.parse('https://www.facebook.com/pokharacentre'); // Replace with your Facebook page URL
    if (await canLaunchUrl(facebookUrl)) {
      await launchUrl(facebookUrl);
    }
    // No print or else block here
  },
                        child:
                      FaIcon(
    ...
    // No print or else block here
                    ...
                    child:   FaIcon(
                       ...
                       InkWell(
                        onTap: () async {
    final Uri facebookUrl = Uri.parse('https://www.instagram.com/pokhararesearchcentre/'); // Replace with your Facebook page URL
    if (await canLaunchUrl(facebookUrl)) {
      await launchUrl(facebookUrl);
    }
    // No print or else block here
  ...
  child: Container(
    ...
    child: Image.asset(
      ...
      filterQuality: FilterQuality.high,
            ...
            width: double.infinity,
           child: Center(child: Text("Provincial Government Page", style: TextStyle(fontSize: 30, fontWeight: FontWeight.bold),)),
            ),
            Container(
